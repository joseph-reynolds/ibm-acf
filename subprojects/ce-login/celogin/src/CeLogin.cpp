...
CeLogin::CeLoginRc CeLogin::getServiceAuthorityV1(const uint8_t* accessControlFileParm, const uint64_t accessControlFileLengthParm,
                                                  const uint8_t* passwordParm, const uint64_t passwordLengthParm,
                                                  ...
                                                  CeLogin::ServiceAuthority& authorityParm)
{
    ...
    if(CeLoginRc::Success == sRc)
    {
        ...
        sRc = decodeAndVerifyAcf(accessControlFileParm, accessControlFileLengthParm,
                                 publicKeyParm, publicKeyLengthParm,
                                 sDecodedAsn);
    }


    // Verify system serial number is in machine list (and get the authorization)
    if(CeLoginRc::Success == sRc)
    {
        sRc = decodeJson((const char*)sDecodedAsn->sourceFileData->data, sDecodedAsn->sourceFileData->length,
    ...
    if(CeLoginRc::Success == sRc)
    {
        ...
        else
        {
            ...
            if(CeLoginRc::Success == sRc)
            {
                // returns a pointer to a time structure or NULL if an error occurred.
                ASN1_TIME* sResult = ASN1_TIME_set(sAsn1CurTime, timeSinceUnixEpocInSecondsParm);
            ...
            // returns -1 if a is before b, 0 if a equals b, or 1 if a is after b. -2 is returned on error.
            const int sCompareResult = ASN1_TIME_compare(sAsn1CurTime, sAsn1ExpirationTime);
        ...
        if(sAsn1CurTime) ASN1_TIME_free(sAsn1CurTime);
        if(sAsn1ExpirationTime) ASN1_TIME_free(sAsn1ExpirationTime);
    }
    ...
    if(CeLoginRc::Success == sRc)
    {
        if(0 != CRYPTO_memcmp(sPasswordHash, sJsonData->mHashedAuthCode, CeLogin_PasswordHashLength))
        {
    ...
    if(sDecodedAsn) CELoginSequenceV1_free(sDecodedAsn);
    if(sJsonData) delete sJsonData;
